...
// Unit examples must not inherit the actual portfolio's site identity or
// custom card. Explicit fixture cwd values below still take precedence.
const EMPTY_WORKSPACE = mkdtempSync(join(tmpdir(), "grok-head-fixture-"));
after(() => rmSync(EMPTY_WORKSPACE, { recursive: true, force: true }));
const injectGrokPwaHead = (html, ctx = {}) => injectHead(html, { cwd: EMPTY_WORKSPACE, ...ctx });
const createHeadInjector = (ctx = {}) => createInjector({ cwd: EMPTY_WORKSPACE, ...ctx });
