...
- Existing and new Node test suites: 294 passed, 0 failed, 4 skipped (298 total); 4 optional host-template document tests are explicitly skipped because those non-versioned instruction files are absent. No runtime behavior test is counted as passed when skipped.
...
Run 35068818354 passed all four dev routes at four widths, production-home comparison and desktop/mobile operations including real backup export/restore on both Node versions. Spatial tests exposed ambiguous select labels and an asynchronous renderer initialization failure escaping React boundaries when WebGL was absent. Explicit bilingual labels and a resource-releasing WebGL2 guard now address those failures. Three unit regressions were added, along with actual context-loss browser coverage. This new source revision awaits its own completed CI result.
